             ...
             <ul className="rounded">
              {data.map((item) => (  
                    <Chatlistview
                        key={item.UserID}  
                        index={item.UserID}
                        Selected={(item.UserID == props.currentChat) ? true  : false}
                        lastSentMessage={item.lastMessage}
                        ProfileImage={`https://randomuser.me/api/portraits/med/men/${item.UserID}.jpg`}
